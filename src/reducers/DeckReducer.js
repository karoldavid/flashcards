...
export default function decks(state = initialDecks, action) {
  switch (action.type) {
    ...
    case SELECT_DECK:
      return {
        ...state,
        selected: state.deckList.filter(
          deck => deck.title === action.payload.title
        )[0]
      };
    case ADD_DECK:
      const deck = {
    ...
    case ADD_CARD:
      const currentDeck = state.deckList.filter(
      ...
      return {
        ...
        deckList: state.deckList.map(
          deck => (deck.title === action.title ? currentDeck : deck)
        )
      };
    case DELETE_DECK:
      return state;
